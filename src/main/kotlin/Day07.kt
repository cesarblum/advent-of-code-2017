...
object Day07 {
    data class Program(val name: String, val weight: Int, val programsAbove: List<String>)

    data class AnnotatedProgram(val program: Program, val cumulativeWeight: Int, val annotatedProgramsAbove: List<AnnotatedProgram>) {
        val balanced: Boolean = annotatedProgramsAbove.map { it.cumulativeWeight }.distinct().count() == 1
    }

    data class UnbalancedProgram(val program: Program, val expectedWeight: Int)

    fun solvePart1() = findBaseProgram(input()).name

    fun solvePart2() = findUnbalancedProgram(input()).expectedWeight

    fun findBaseProgram(input: String): Program = findBaseProgram(parseInput(input))

    private fun findBaseProgram(programs: List<Program>): Program =
        programs.single { program -> allEdges(programs).none { it.second == program.name } }

    fun findUnbalancedProgram(input: String): UnbalancedProgram =
        findUnbalancedProgram(parseInput(input))

    private fun findUnbalancedProgram(programs: List<Program>): UnbalancedProgram =
        findUnbalancedProgram(annotate(programs))

    private fun findUnbalancedProgram(program: AnnotatedProgram): UnbalancedProgram =
        findUnbalancedProgram(program, 0)

    private fun findUnbalancedProgram(program: AnnotatedProgram, expectedCumulativeWeight: Int): UnbalancedProgram =
        if (program.balanced) {
            UnbalancedProgram(
                program.program,
                program.program.weight - (program.cumulativeWeight - expectedCumulativeWeight))
        } else {
            program.annotatedProgramsAbove
                .groupBy { it.cumulativeWeight }
                .entries
                .map { it.value }
                .partition { it.size == 1 }
                .let {
                    val unbalancedProgramAbove = it.first.first().first()
                    val nextExpectedCumulativeWeight = it.second.first().first().cumulativeWeight
                    findUnbalancedProgram(unbalancedProgramAbove, nextExpectedCumulativeWeight)
                }
        }

    private fun allEdges(programs: List<Program>): List<Pair<String, String>> =
        programs.flatMap(this::edges)

    private fun edges(program: Program): List<Pair<String, String>> =
        program.programsAbove.map { Pair(program.name, it) }

    private fun annotate(programs: List<Program>) =
        annotate(findBaseProgram(programs), programs.associateBy { it.name })

    private fun annotate(program: Program, programGraph: Map<String, Program>): AnnotatedProgram {
        if (program.programsAbove.isEmpty()) {
            return AnnotatedProgram(program, program.weight, emptyList())
        }

        return program.programsAbove
            .map { programName -> annotate(programGraph[programName]!!, programGraph) }
            .let { annotatedProgramsAbove ->
                AnnotatedProgram(
                    program,
                    program.weight + annotatedProgramsAbove.sumBy { it.cumulativeWeight },
                    annotatedProgramsAbove)
            }
    }

    fun parseProgram(input: String): Program {
        val pattern = Regex("^(\\w+) \\((\\d+)\\)( -> (\\w+(, \\w+)*))?$")
        val result = pattern.find(input)!!

        return Program(result.groupValues[1], result.groupValues[2].toInt(),
            if (result.groupValues.size > 3) {
                result.groupValues[4].split(", ").filter { it.isNotEmpty() }
            } else {
                emptyList()
            })
    }

    private fun parseInput(input: String): List<Program> = input.lines().map(this::parseProgram)

    private fun input() = """hndzyx (28)
kdelzk (399)
