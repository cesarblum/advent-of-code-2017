...
object Day08 {
    ...
    class RegisterBank {
        ...
        operator fun get(register: String) = registers.getOrPut(register, { 0 })

        operator fun set(register: String, value: Int) {
            registers[register] = value
        }

        fun toMap(): Map<String, Int> = registers
    }

    class Instruction(private val register: String,
                      private val registerOperation: RegisterOperation,
                      private val value: Int,
                      private val comparison: Comparison) {
        fun execute(registerBank: RegisterBank): RegisterBank {
            if (comparison.execute(registerBank)) {
                when (registerOperation) {
                    RegisterOperation.INCREMENT -> registerBank[register] += value
                    RegisterOperation.DECREMENT -> registerBank[register] -= value
                }
            }
    ...
    class Comparison(private val register: String,
                     private val comparisonOperation: ComparisonOperation,
                     private val value: Int) {
        fun execute(registerBank: RegisterBank): Boolean =
            when (comparisonOperation) {
                ComparisonOperation.EQUALS -> registerBank[register] == value
                ComparisonOperation.NOT_EQUALS -> registerBank[register] != value
                ComparisonOperation.LESS_THAN -> registerBank[register] < value
                ComparisonOperation.LESS_THAN_OR_EQUALS -> registerBank[register] <= value
                ComparisonOperation.GREATER_THAN -> registerBank[register] > value
                ComparisonOperation.GREATER_THAN_OR_EQUALS -> registerBank[register] >= value
            }
    }

    fun solvePart1(): Int = findLargestRegisterValue(input())

    fun solvePart2(): Int = findHighestSetValue(input())

    fun findLargestRegisterValue(input: String): Int =
        findLargestRegisterValue(parseInput(input))

    private fun findLargestRegisterValue(instructions: List<Instruction>): Int =
        executeInstructions(instructions).let { registerBank -> registerBank.toMap().values.max() ?: 0 }

    fun findHighestSetValue(input: String) =
        findHighestSetValue(parseInput(input))

    private fun findHighestSetValue(instructions: List<Instruction>): Int {
        var highestSetValue = 0

        executeInstructions(instructions, { registerBank ->
            highestSetValue = Math.max(highestSetValue, registerBank.toMap().values.max() ?: highestSetValue)
        })

        return highestSetValue
    }

    private fun executeInstructions(instructions: List<Instruction>): RegisterBank =
        executeInstructions(instructions, {})

    private fun executeInstructions(instructions: List<Instruction>,
                                    onInstructionExecuted: (RegisterBank) -> Unit): RegisterBank =
        instructions.fold(RegisterBank(), { registerBank, instruction ->
            instruction.execute(registerBank)
            onInstructionExecuted(registerBank)
            return@fold registerBank
        })

    private fun parseInput(input: String): List<Instruction> =
        input.lines().filter { it?.isNotEmpty() }.map(this::parseInstruction)

    private fun parseInstruction(input: String): Instruction {
        val pattern = Regex("^(\\w+) (inc|dec) (-?\\d+) if (\\w+) (==|!=|<|<=|>|>=) (-?\\d+)$")
        val result = pattern.find(input) ?: throw IllegalArgumentException("Unable to parse instruction from '$input'")

        val register = result.groupValues[1]
        val registerOperation = when (result.groupValues[2]) {
            "inc" -> RegisterOperation.INCREMENT
            "dec" -> RegisterOperation.DECREMENT
            else -> throw IllegalArgumentException("'${result.groupValues[2]}' is not a valid register operation }")
        }
        val value = result.groupValues[3].toInt()
        val comparisonRegister = result.groupValues[4]
        val comparisonOperator = when (result.groupValues[5]) {
            "==" -> ComparisonOperation.EQUALS
            "!=" -> ComparisonOperation.NOT_EQUALS
            "<" -> ComparisonOperation.LESS_THAN
            "<=" -> ComparisonOperation.LESS_THAN_OR_EQUALS
            ">" -> ComparisonOperation.GREATER_THAN
            ">=" -> ComparisonOperation.GREATER_THAN_OR_EQUALS
            else -> throw IllegalArgumentException("'${result.groupValues[5]}'is not a valid comparison operator")
        }
        val comparisonValue = result.groupValues[6].toInt()

        return Instruction(register, registerOperation, value,
            Comparison(comparisonRegister, comparisonOperator, comparisonValue))
    }

    private fun input() = """j inc -19 if jhb >= 10
es inc -432 if gbu <= -5
es inc 278 if ib > -9
es inc -835 if ib >= -6
q dec 420 if buw == -2
